...
class scDART(object):
    ...
    def fit(self, rna_count, atac_count, reg, rna_anchor = None, atac_anchor = None):
        ...
        indices = torch.tensor([indexes[0], indexes[1]], dtype=torch.long)
        values = torch.tensor(reg.data, dtype=torch.float32)
        shape = torch.Size(reg.shape)
        coarse_reg = torch.sparse.FloatTensor(indices, values, shape)

        # batch_size = int(max([len(self.rna_dataset),len(self.atac_dataset)])/4) if self.batch_size is None else self.batch_size
        batch_size = int(min([self.rna_dataset.shape[0],self.atac_dataset.shape[0]])/4)
