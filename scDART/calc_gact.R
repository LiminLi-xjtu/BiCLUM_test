...
# 
# regions <- read.table("/Users/guoyin/Rproject/gene_score/data_with_meta/1_SHARE_mouse_GSE140203/additional_file/dscATAC-seq/hg19-tss.bed.txt")

#############################################################################
# hyper-parameters
species <- "Mouse"

path <- "/Users/guoyin/Rproject/gene_score/data_with_meta/"

library("zellkonverter")
sce <- readH5AD(paste0(path,"1_SHARE_mouse_GSE140203/GASM/brain/rna.h5ad"))
data.rna = sce@assays@data@listData[["X"]]

regions <- read.table(paste0(path,"1_SHARE_mouse_GSE140203/raw_data/brain/GSM4156599_brain.peaks.bed"))
colnames(x = regions) <- c("chromosome", "start", "end")

peak_regions = paste(paste(regions[,1], regions[,2], sep=":"), regions[,3], sep = "-")
...
if(species == "Mouse"){
    ...
    A = find_geneact(peak.df = regions, annotation.file = "Homo_sapiens.GRCh38.109.gtf", 
...
tmp = match(unique(A$gene.name), rownames(data.rna))
tmp = na.omit(tmp)
gene_names = rownames(data.rna)[tmp]
...
SaveH5Seurat(scgeneactivity, filename = paste0('../datasets/scDART/', data_id, '.h5Seurat'), overwrite=T)
Convert(paste0('../datasets/scDART/', data_id, '.h5Seurat'), dest = "h5ad")
