...
class scDART(object):
    ...
    def fit(self, rna_count, atac_count, reg, rna_anchor=None, atac_anchor=None):
        ...
        coarse_reg = torch.FloatTensor(reg.toarray()).to(self.device)
        ...
        gene_act = model.gene_act(features=EMBED_CONFIG["gact_layers"], dropout_rate=0.0, negative_slope=0.2).to(
            self.device)
        encoder = model.Encoder(features=EMBED_CONFIG["proj_layers"], dropout_rate=0.0, negative_slope=0.2).to(
            self.device)
        ...
        train.match_latent(model=self.model_dict, opts=opt_dict, dist_atac=dist_atac, dist_rna=dist_rna,
                           data_loader_rna=train_rna_loader, data_loader_atac=train_atac_loader,
                           n_epochs=self.n_epochs + 1,
                           reg_mtx=coarse_reg, reg_d=self.reg_d, reg_g=self.reg_g, reg_mmd=self.reg_mmd,
                           use_anchor=self.use_anchor, norm="l1",
    ...
    def transform(self, rna_count, atac_count, rna_anchor=None, atac_anchor=None):
        ...
        with torch.no_grad():
            ...
            for data in test_atac_loader:
                self.z_atac = self.model_dict["encoder"](
                    self.model_dict["gene_act"](data['count'].to(self.device))).cpu().detach()
    ...
    def fit_transform(self, rna_count, atac_count, reg, rna_anchor=None, atac_anchor=None):
        ...
        batch_size = int(
            max([len(self.rna_dataset), len(self.atac_dataset)]) / 4) if self.batch_size is None else self.batch_size
        ...
        if mode == "backbone" or mode == "pseudotime":
            ...
            if mode == "backbone":
                utils.plot_backbone(ae_rna, ae_atac, mode="joint", mean_cluster=mean_cluster, groups=groups, T=T,
                                    figsize=_kwargs['fig_size'], save=save_path, anno1=rna_anno, anno2=atac_anno,
                                    axis_label="PCA")
            if mode == "pseudotime":
